...
variables = ["persistent_absence_percent", "Mean", "IMD"]
labels = ["Persistent Absence (%)", "Mean Income", "IMD Rank"]
...
plt.tight_layout()
plt.savefig("./SSA5/multivariable_correlation6.png")

for year in [2024, 2025]:
    data = merged[merged["Year"] == year].dropna(subset=["persistent_absence_percent", "Crime Rate", "Mean", "IMD"])
    
    X = data[["persistent_absence_percent", "Mean", "IMD"]].values
    y = data["Crime Rate"].values
    ...
    print(f"  IMD coefficient:   {model.coef_[2]:.4f}")
